...
def plot_layer_ax(obj, model_name):
    ...
    if args.savedir:
        ...
        plt.savefig(os.path.join(args.savedir, f"{model_name}_layer_layers.pdf"), bbox_inches="tight", dpi=300)
...
#%%
# 绘制激活通道索引图
plot_layer_channel(activation_channel_indices, args.model, ylabel="Activation Channel Index", filename="layer_features", ylim=feat_abs.shape[-1])
...
def plot_3d_feat_sub(ax, obj, layer_id, model_name, config, savedir=None):
    ...
    if savedir:
        ...
        plt.savefig(os.path.join(savedir, f"{model_name}_layer_seqs.pdf"), bbox_inches="tight", dpi=300)
...
stats = {}
seq = "Are you sure? Yes, absolutely!"
valenc = tokenizer(seq, return_tensors='pt', add_special_tokens=False).input_ids.to(device)
