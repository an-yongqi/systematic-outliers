...
from lib.model_dict import MODEL_TITLE_DICT, WEIGHT_DICT, DOWN_INPUT_DICT
...
args = SimpleNamespace(
    ...
    revision="main",
    access_token=None,
    savedir="figures/mlp_relation_end"
)
...
def plot_3d_weight_sub(ax, weight, selected_indices, layer_id, model_name, config, weight_key, savedir=None):
    ...
    # 动态生成配置
    top_1 = config["top_1"]
    top_1_index = [selected_indices.index(i) for i in top_1]
    
    # 填充省略号
    xticklabels_full = ["···"] * num_columns
    xticklabels_full[0] = '0'
    for i in range(len(top_1_index)):
        xticklabels_full[top_1_index[i]] = str(selected_indices[top_1_index[i]])
    ...
    # 绘制 heavy 部分
    ax.plot_wireframe(xdata[:, top_1_index], ydata[:, top_1_index], zdata[:, top_1_index], rstride=0, color="magenta", linewidth=3)
...
config = {"top_1": [3721, 7006],
          "yticks": ([1415, 2533], [1415, 2533]),
          "layer_id": 1,
          "selected_indices": [0, -1, -1, 3720, 3721, 3722, -1, 7005, 7006, 7007, -1],
    }
layer_id = 30
...
def plot_3d_feat_sub(ax, obj, layer_id, model_name, config, act_keys, savedir=None):
    ...
    if num_channels == 11008:
        ax.set_yticks(*config["yticks"], fontsize=15, fontweight="heavy")
    elif key in ["down_out", "rms_in", "rms_out", "layer_out"]:
